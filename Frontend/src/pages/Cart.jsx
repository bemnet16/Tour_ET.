...
function Cart() {
  ...
  const showItems = state.map(item => {
    return (
      ...
      <div className="  row shadow p-3 bg-light align-items-center my-2" key={item._id}>
        <div className="col-md-2"><img src={item.photo} alt="" className='img-fluid' /></div>
        ...
        <div className="btns col-md-4">
          <button className='m-3 btn btn-outline-primary' onClick={() => { removeFromcart(item) }}>remove</button>
          <Link className='btn btn-outline-primary' to={`/book/${item.packages}`} key={item.packages}>Book</Link>
        </div>
      </div>
      ...
      <div className="container-md  pt-5">
        <div className="row gap-3 align-items-start ">
          ...
          <div className='col-md-3 card shadow p-3 bg-dark'>
            <h3 className='h4 text-start text-white fw-bold'>Review Order Details</h3>
